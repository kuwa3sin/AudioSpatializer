...
Spatializer APIでHRTF(頭部伝達関数)による処理を行い、自然なサラウンド音声として再生します。
...
- その他、Android Head Tracking対応デバイス

### 機能が一部利用できない可能性のある機種・組み合わせ

- Samsung Galaxy
  - Android Spatial Audioには対応しており、5.1chの仮想化再生は可能
  - ヘッドトラッキングは独自実装のため、上記対応デバイスと接続した場合では利用不可(Galaxy Budsシリーズなら有効化可能だが、動作は未検証)
...
- **Material 3**: 角丸のカードデザイン、フローティングバー上のコントロール
- **Dynamic Color**: 壁紙に合わせたテーマカラー
- **ダークモードに対応**: システム設定に連動
